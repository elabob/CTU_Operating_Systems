...
bool has_active_worker_for_place(place p) {
    for (auto w : workers) {
        if (w->profession == p && !w->should_end && !w->exited) {
            return true;
        }
    }
    return false;
...
bool can_part_reach_place(Part part, place target) {
    for (int step = part.step; step < 6; step++) {
        if (workflows[part.prod][step] == target) {
            for (int prev_step = part.step; prev_step < step; prev_step++) {
                place needed = (place)workflows[part.prod][prev_step];
                if (places[needed] == 0 || !has_active_worker_for_place(needed)) {
                    return false;
                }
            }
            return true;
...
bool is_profession_needed(place prof) {
    if (places[prof] == 0) return false;
    for (auto &p : parts) {
        if (can_part_reach_place(p, prof)) {
            return true;
        }
    }
    return false;
...
void* worker_func(void* arg) {
    ...
    while (true) {
        ...
        while (true) {
            ...
            if (eof_received && !is_profession_needed(w->profession)) {
                w->exited = true;
                pthread_cond_broadcast(&cond);
                pthread_mutex_unlock(&mtx);
                printf("%s goes home\n", w->name.c_str());
            ...
            int best_idx = -1;
            int best_step = -1;
            product best_prod = (product)_PRODUCT_COUNT;

            for (size_t i = 0; i < parts.size(); i++) {
                if (workflows[parts[i].prod][parts[i].step] == w->profession && places[w->profession] > 0) {
                    if (best_idx == -1 ||
                        parts[i].step > best_step ||
            ...
            if (best_idx >= 0) {
                ...
                printf("%s %s %d %s\n", w->name.c_str(), place_str[w->profession], p.step + 1, product_str[p.prod]);
