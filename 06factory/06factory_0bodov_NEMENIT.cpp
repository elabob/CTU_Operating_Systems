...
bool can_profession_work(place prof) {
    if (places[prof] == 0) return false;
    for (auto &p : parts) {
        if (workflows[p.prod][p.step] == prof) return true;
    }
    return false;
}

bool should_worker_leave(Worker *w) {
    if (!eof_received) return false;
    if (!can_profession_work(w->profession)) return true;
    return false;
}
